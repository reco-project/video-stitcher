...
class LensProfileModel(BaseModel):
    ...
    camera_matrix: dict = Field(
        ..., 
        description="Camera intrinsic matrix {fx, fy, cx, cy}"
    )
    distortion_coeffs: list[float] = Field(
        ..., 
        description="Distortion coefficients (4 floats for fisheye_kb4)"
    )
    ...
    metadata: Optional[dict] = Field(
        None, 
        description="Optional provenance info (calibrated_by, calibration_date, notes)"
    )
    ...
    def validate_distortion_coeffs(cls, v: list[float]) -> list[float]:
        ...
        if len(v) != 4:
            raise ValueError(
                "distortion_coeffs must contain exactly 4 values for fisheye_kb4 model"
            )
