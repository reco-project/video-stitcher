...
/// GPU-accelerated variant of `process_undistorted_pair`.
///
/// Uses GPU compute shaders for AKAZE diffusion (the 99% bottleneck),
/// then CPU for everything else.
#[allow(clippy::too_many_arguments)]
fn process_undistorted_pair_gpu(
    left_rgba: &[u8],
    right_rgba: &[u8],
    lw: u32,
    lh: u32,
    ...
    profile_scope!("process_frame_gpu");
    ...
    // Use GPU-accelerated AKAZE extraction
    let (kp_left, desc_left) = {
        profile_scope!("akaze_gpu_left");
        features::detect_gpu(
            left_rgba,
            lw,
            lh,
    ...
    log::debug!(
        "frame {frame_idx}: {} left keypoints, {} right keypoints (GPU)",
        kp_left.len(),
        kp_right.len()
    );
    ...
    // The rest is identical to the CPU path (matching, filtering, normalization)
    let raw_matches = features::match_descriptors(&desc_left, &desc_right, config.lowe_ratio);
    let post_ratio_test = raw_matches.len();
...
pub fn calibrate(
    ...
    // GPU diffusion infrastructure exists (gpu_diffusion.rs) but current
    // per-dispatch overhead makes it slower than CPU parallelism. Using CPU
    // path with rayon until GPU batching is implemented.
