...
use super::{Akaze, DESC_BYTES, KeyPoint};

impl Akaze {
    /// Extract M-LDB binary descriptors for all keypoints.
    ...
    /// Compute the rotation-invariant M-LDB binary descriptor for one keypoint.
    fn get_mldb_descriptor(
        &self,
        keypoint: &KeyPoint,
        evolutions: &[EvolutionStep],
    ) -> [u8; DESC_BYTES] {
        let mut output = [0u8; DESC_BYTES];
...
/// Binary comparisons to build the descriptor.
