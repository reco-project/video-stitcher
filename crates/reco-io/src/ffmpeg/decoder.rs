...
fn try_hwaccel(
    ...
    // CUDA first: on NVIDIA, CUDA decode feeds the CUDA→DX12 zero-copy
    // path (Windows) or CUDA→Vulkan (Linux). On AMD/Intel, CUDA fails
    // and D3D11VA/VAAPI is used instead.
    let candidates = [
        (
