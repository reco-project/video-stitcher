use reco_core::calibration::{Calibration, FieldRoi, Lens};
...
pub(crate) fn scene_for_calibration(calibration: &Calibration) -> SceneGeometry {
    let scene_aspect =
        calibration.lenses[0].width as f32 / calibration.lenses[0].height.max(1) as f32;
    SceneGeometry::for_calibration(calibration, scene_aspect)
}
