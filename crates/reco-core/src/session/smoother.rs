//! Bidirectional One Euro trajectory smoother.
//!
...
///
/// Given a window of raw `ViewportPosition`s from the lookahead buffer,
/// applies a forward One Euro pass, then a backward One Euro pass,
/// and returns the smoothed position for the oldest frame (the one
...
/// single pass).
pub(crate) struct TrajectorySmoother {
...
mod tests {
    ...
    fn smoothing_preserves_linear_trend() {
        ...
        // The bidirectional average pulls it slightly above 0.0 but well
        // below the midpoint (0.07).
        assert!(
            smoothed.yaw < 0.05,
