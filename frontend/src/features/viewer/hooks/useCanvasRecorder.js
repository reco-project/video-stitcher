 ...
 */
export function useCanvasRecorder({ fps = 30, mimeType = 'video/webm;codecs=vp9', videoBitsPerSecond = 5000000 } = {}) {
	...
	const startRecording = useCallback(
		(canvas, audioSource = null, micStream = null) => {
			if (!canvas) {
				...
				if (audioSource && audioSource.captureStream && micStream) {
					...
					if (videoAudioTracks.length > 0) {
						const videoSource = audioContext.createMediaStreamSource(
							new MediaStream([videoAudioTracks[0]])
						);
