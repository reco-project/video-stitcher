(function universalScoreboardDesigner() {
    ...
    function updateEditingUi() { elements.canvas.classList.toggle("editing-guides-hidden", !editingEnabled); elements.canvas.dataset.editingGuides = editingEnabled ? "visible" : "hidden"; elements.toggleEditing.textContent = editingEnabled ? "Hide editing guides" : "Show editing guides"; for (const button of elements.canvasToolbar.querySelectorAll("[data-select-target]")) button.classList.toggle("selected", button.dataset.selectTarget === selectedTarget); for (const element of [elements.card, elements.freeText, elements.freeLogo]) element.classList.toggle("is-selected", element.dataset.dragTarget === selectedTarget); }
